...
## Dernière évolution : boosters automatiques

Le score comportemental quotidien reste calculé depuis les événements, mais `v_daily.gained` et `v_daily.lost` excluent les boosters, les anciens bonus de régularité, les récompenses et les contrepassations. Les colonnes `boosters` et `daily_score` sont ajoutées en fin de vue pour préserver le contrat existant.

Le trigger `events_auto_boosters` appelle `grant_period_boosters` après une saisie bonus, malus, réparation ou ajustement. La fonction calcule les scores journaliers plafonnés à zéro depuis le début de la semaine ou du mois, puis crée un événement `booster` séparé. L'unicité est garantie par `booster_grants(family_id, child_id, period_type, period_start)`.

Les règles sont modifiables dans Réglages. La page Enfant affiche la progression vers les boosters actifs et les boosters déjà gagnés.
