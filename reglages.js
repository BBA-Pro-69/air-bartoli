...
let root = null;
let children = [], cats = [], rewards = [], special = [], cinematic = null, boosters = [], famille = null;
...
async function reload() {
  [children, cats, rewards, special, cinematic, boosters] = await Promise.all([
    api.getChildren(), api.getCategories(), api.getRewards(), api.getSpecialDays(), api.getCinematicSettings(), api.getBoosterSettings()]);
  render();
}
...
function render() {
  ...
  // --- boosters automatiques de régularité
  const boosterCard = el('div', { class: 'card' },
    el('h2', {}, 'Boosters automatiques'),
    el('p', { class: 'muted', style: 'margin-top:-6px' },
      'Les points gagnés dans les journées restent inchangés. Lorsqu’un seuil est atteint, un booster séparé est créé une seule fois pour l’enfant et la période.'),
    el('p', { class: 'booster-rule-help' },
      'Calcul : booster = plafond supérieur(score de la période × coefficient) − score de la période. Les anciens boosters ne comptent jamais dans le seuil.'),
    ...['week', 'month'].map(type => {
      const b = boosters.find(x => x.period_type === type) || { period_type: type, active: false, min_points: type === 'week' ? 35 : 120, multiplier: type === 'week' ? 2 : 1.5 };
      const active = el('input', { type: 'checkbox', checked: !!b.active, style: 'width:auto;min-height:auto' });
      const min = el('input', { type: 'number', min: '1', step: '1', value: String(b.min_points) });
      const mult = el('input', { type: 'number', min: '1', max: '9.9', step: '0.1', value: Number(b.multiplier).toFixed(1) });
      const title = type === 'week' ? 'Semaine calendaire' : 'Mois calendaire';
      const desc = type === 'week' ? 'Du lundi au dimanche.' : 'Du 1er au dernier jour du mois.';
      return el('div', { class: 'booster-setting' },
        el('div', { class: 'row booster-setting-head' },
          el('label', { class: 'row booster-toggle' }, active, el('strong', {}, title)),
          el('span', { class: 'muted' }, desc)),
        el('div', { class: 'fields' },
          champ('Seuil de points', min), champ('Coefficient ×', mult)),
        el('button', { class: 'btn btn-primary btn-sm', onclick: async () => {
          const threshold = Number(min.value), coefficient = Number(mult.value);
          if (!Number.isInteger(threshold) || threshold < 1 || !Number.isFinite(coefficient) || coefficient < 1 || coefficient > 9.9 || Math.abs(Math.round(coefficient * 10) - coefficient * 10) > 1e-9) {
            fail(new Error('Le seuil doit être entier et le coefficient doit avoir au maximum une décimale.'));
            return;
          }
          try {
            await api.save('booster_settings', { family_id: famille, period_type: type, active: active.checked, min_points: threshold, multiplier: coefficient });
            const n = await api.evaluateCurrentBoosters();
            await reload();
            toast(n ? 'Booster enregistré, ' + n + ' booster' + (n > 1 ? 's' : '') + ' accordé' + (n > 1 ? 's' : '') + '.' : 'Booster enregistré.');
          } catch (e) { fail(e); }
        }}, 'Enregistrer'));
    }),
    el('p', { class: 'muted', style: 'margin-bottom:0' },
      'Le déclenchement se fait automatiquement après chaque saisie. Il peut aussi être vérifié immédiatement avec Enregistrer.'));
  app.append(boosterCard);
}
