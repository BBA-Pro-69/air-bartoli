...
function boosterEvents(from = null, to = null) {
  return events.filter(e => (e.kind === 'booster' || e.kind === 'bonus_streak') &&
    (!filter || e.child_id === filter) && (!from || e.event_date >= from) && (!to || e.event_date <= to));
}

function renderBoosterStrip(from = null, to = null) {
  const list = boosterEvents(from, to);
  if (!list.length) return el('div', { class: 'booster-journal-empty muted' }, 'Aucun booster gagné sur cette période.');
  return el('div', { class: 'booster-journal-list' }, ...list.map(e => {
    const c = child(e.child_id);
    return el('div', { class: 'booster-journal-item', style: `--kid:${c?.color || 'var(--cyan)'}` },
      el('div', {}, el('strong', {}, c?.first_name || 'Famille'), el('span', { class: 'muted' }, ' · ' + shortDay(e.event_date))),
      el('span', { class: 'entry-pts pos' }, pts(e.points)),
      el('div', { class: 'booster-journal-note' }, e.note || 'Booster de régularité'));
  }));
}
...
function renderCalendar() {
  ...
  return el('section', { class: 'journal-calendar-card card' },
    ...
    el('div', { class: 'journal-legend' },
      ...
      el('span', {}, '· = rien saisi')),
    el('div', { class: 'booster-journal-card' },
      el('div', { class: 'booster-journal-title' }, 'Boosters gagnés sur le mois'),
      renderBoosterStrip(start, end)));
}
...
function renderToday() {
  ...
  const dayEvents = events.filter(e => e.event_date === day && !['booster','bonus_streak'].includes(e.kind) && (!filter || e.child_id === filter));
  ...
  return el('section', { class: 'journal-today-card' },
    ...
    el('div', { class: 'booster-journal-card' },
      el('div', { class: 'booster-journal-title' }, 'Boosters gagnés ce jour-là'),
      renderBoosterStrip(day, day)));
}
...
function openDay(day) {
  const rows = dailyMapFor(day);
  const list = events.filter(e => e.event_date === day && !['booster','bonus_streak'].includes(e.kind) && (!filter || e.child_id === filter));
  const flags = eventFlags(events);
  const body = el('div', {},
    ...
    el('div', { class: 'booster-journal-card' },
      el('div', { class: 'booster-journal-title' }, 'Boosters'),
      renderBoosterStrip(day, day)));
